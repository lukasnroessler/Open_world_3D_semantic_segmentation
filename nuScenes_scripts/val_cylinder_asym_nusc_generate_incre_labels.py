...
def main(args):
    ...
    with torch.no_grad():
        for i_iter_val, (_, val_vox_label, val_grid, val_pt_labs, val_pt_fea, idx) in enumerate(val_dataset_loader):
            ...
            point_predict.tofile("/harddisk/jcenaa/nuScenes/predictions/predictions_base_train/" + idx_s + ".label")

            pred = np.fromfile(
                "/harddisk/jcenaa/nuScenes/predictions/predictions_base_train/" + idx_s + ".label", dtype=int32
            )
            unknown_clss = [1, 5, 8, 9]
            for unknown_cls in unknown_clss:
